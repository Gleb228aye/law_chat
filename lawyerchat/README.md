...
Контрольные вопросы находятся в `backend/data/evaluation/retrieval_cases.json`. Текущий набор содержит 60 автоматически оцениваемых вопросов: по 10 вопросов для каждого из шести законов, а также отдельные out-of-scope кейсы без строгой retrieval-оценки. Кейсы используют `expected_sources` с уровнями `primary`, `secondary` и `acceptable`; legacy-поле `expected_article_numbers` также поддерживается. Оценка обращается к PostgreSQL + pgvector и не вызывает LLM.

Запуск semantic-режима из папки `backend`:
...
- `Recall@K` — доля вопросов, для которых правильная статья нужного закона найдена среди первых K результатов.
- `MRR` — среднее обратное значение позиции первого правильного результата; ненайденные статьи дают нулевой вклад.
- `Mean Rank` — средняя позиция первого правильного результата среди вопросов, где статья была найдена.
- `Document Recall@5` — доля вопросов, для которых нужный нормативный документ присутствует в первых пяти результатах независимо от номера статьи.
- `Wrong Document@1` — доля вопросов, у которых первый результат относится к другому нормативному документу.

Метрики группируются по законам и по `question_type`. Источники с релевантностью `primary` и `secondary` считаются полноценным совпадением; `acceptable` отмечается как `partial_match`, но не увеличивает Recall@K или MRR. Out-of-scope кейсы выводятся отдельно без строгой автоматической оценки. Если `top_k` меньше глубины метрики, отчёт отмечает её как не рассчитанную.

Отчёты сохраняются в указанной папке:
...
LawyerChat поддерживает два режима retrieval:

- `semantic` — поиск только по близости embedding-векторов в pgvector;
- `hybrid` — сочетание semantic search, полнотекстового поиска PostgreSQL и metadata boost по статье и нормативному документу.

Режим можно выбрать во frontend перед отправкой вопроса. Один и тот же переключатель применяется к основному чату и техническому поиску. По умолчанию используется `hybrid`; старые API-запросы без `retrieval_mode` продолжают использовать режим из backend-настройки `RETRIEVAL_MODE`.

## Локальная история диалога

История чата сохраняется в `localStorage` браузера. В неё входят вопросы пользователя, ответы ассистента, отображаемые источники и режим retrieval, которым был получен ответ. После перезагрузки страницы сообщения, режим и карточки источников восстанавливаются без повторных запросов к backend.
